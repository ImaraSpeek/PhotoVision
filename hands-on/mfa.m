...
%
% [LogL,mfa,Q] = mfa(X,d,C,iso,eq,dia);
% 
...
% % Compute responsibilities of points to clusters
% R = zeros(N, C);
% for c=1:C
%     MoFA.W(:,:,c) = mgs(MoFA.W(:,:,c));
%     Xn = bsxfun(@minus, X, MoFA.M(:,c));
%     inv_sigma = inv(MoFA.W(:,:,c) * MoFA.W(:,:,c)' + diag(MoFA.Psi(:,c)));
%     for n=1:N
%         R(n, c) = MoFA.mix(c) * exp(-.5 * Xn(:,n)' * inv_sigma * Xn(:,n));
%     end
% end
% R = bsxfun(@rdivide, R, sum(R, 2));
% 
% % Compute responsibility-weighted covariances
% covs = zeros(D, D, C);
% for c=1:C
%     Xn = bsxfun(@minus, X, MoFA.M(:,c));
%     covs(:,:,c) = Xn * bsxfun(@times, R(:,c), Xn');
% end
% 
% % Compute weights of factors
% weights = zeros(size(covs, 1), size(covs, 3));
% for c=1:C
%     weights(:,c) = eig(covs(:,:,c));
% end
% weights
